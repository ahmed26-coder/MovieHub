...
export async function TrendingMovies() {
  const data = getStaticData<{ results: MovieCard[] }>('trending')
...
export async function PopularMovies() {
  const data = getStaticData<{ results: MovieCard[] }>('popular')
  const movies = data?.results?.slice(0, 6) || []
...
export async function TopRatedMovies() {
  const data = getStaticData<{ results: MovieCard[] }>('top-rated')
  const movies = data?.results?.slice(0, 6) || []
...
export async function UpcomingMovies() {
  const data = getStaticData<{ results: MovieCard[] }>('upcoming')
  const movies = data?.results?.slice(0, 6) || []
...
export async function GenreMarqueeSection() {
